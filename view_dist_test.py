...
if __name__ == "__main__":    
    ...
    while not rospy.is_shutdown():
        ...
        render_pos = [candidate_position + np.array([0.2, 0.0, 0.0]),
                    candidate_position + np.array([-0.2, 0.0, 0.0]),
                    candidate_position + np.array([-0.6, 0.0, 0.0]),
                    candidate_position + np.array([-1.0, 0.0, 0.0]),
                    candidate_position + np.array([-1.4, 0.0, 0.0]),
                    candidate_position + np.array([-1.8, 0.0, 0.0]),
                    candidate_position + np.array([-2.2, 0.0, 0.0]),
                        ]
        max_points = 0
        for candidate_position in render_pos:
            ...
            camera_distance = np.linalg.norm(candidate_position - render_lookat)
            num_matches.append((camera_distance, len(mkpts0)))
    ...
    np.savez('./ros_anymal/ETH_Cerberus_localization/num_matches_dist_seq.npz',
